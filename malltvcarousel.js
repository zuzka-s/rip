...
function launchCarousel(id) {
    ...
    const carousel = document.querySelector(id + " .malltv-carousel-vidboxes-container");
    carousel.scrollLeft = 0; //reset on position 0

    const getCarouselMeasuresObj = () => {
        return {
            selectItem: function () { return document.querySelectorAll(id + ' .video-click') },
            ...
            scrollbarHideStyle: function () {
                return "top: " + (document.querySelector(id + ' .malltv-carousel-hide-scrollbar-wrapper').clientHeight - document.querySelector(id + ' .malltv-carousel-vidboxes-container').clientHeight) + "px;";
            }
        }
    }
